...
local function runUnlockAllLogic()
	...
	if ClientItem and ClientItem._CreateViewModel then
		local originalCreateViewModelDance = ClientItem._CreateViewModel
		ClientItem._CreateViewModel = function(self, viewmodelRef)
			local weaponName = self.Name
			local weaponPlayer = self.ClientFighter and self.ClientFighter.Player
			constructingWeapon = (weaponPlayer == player) and weaponName or nil
			if weaponPlayer == player and equipped[weaponName] and equipped[weaponName].Dance and viewmodelRef then
				local dataKey, danceKey, nameKey = self:ToEnum("Data"), self:ToEnum("Dance"), self:ToEnum("Name")
				if viewmodelRef[dataKey] then
					viewmodelRef[dataKey][danceKey] = equipped[weaponName].Dance
					viewmodelRef[dataKey][nameKey] = equipped[weaponName].Dance.Name
				elseif viewmodelRef.Data then
					viewmodelRef.Data.Dance = equipped[weaponName].Dance
					viewmodelRef.Data.Name = equipped[weaponName].Dance.Name
				end
			end
			local result = originalCreateViewModelDance(self, viewmodelRef)
			constructingWeapon = nil
			return result
		end
	end
