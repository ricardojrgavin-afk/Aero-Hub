...
local function updateFOVCircle()
	...
	if not s.ShowFOV or not Features.aimbotEnabled or s.TargetMode ~= "FOV" then
...
local State = {
	...
	aimbotSettings = { FOV=120, Smoothness=35, TeamCheck=true, TargetPart="Head", ShowFOV=true, Prediction=50, TargetMode="Distance" },
}
...
local function UpdateWhitelistMenu()
	if not WhitelistMenu.Visible then return end
...
brandBlock.Size = UDim2.new(1, 0, 0, IsMobile habits and 42 or 58)
...
local function runUnlockAllLogic()
	...
	DataController.Get = function(self, key)
		...
		if key == "CosmeticInventory" then
			local proxy = {}
			if data then
				for k, v_ in pairs(data) do
					local cosmetic = CosmeticLibrary.Cosmetics[k]
					if cosmetic and cosmetic.Type == "Skin" then proxy[k] = v_ end
				end
			end
	...
	DataController.Get = function(self, key)
		...
		if key == "CosmeticInventory" then
			...
			if data then
				for k, v_ in pairs(data) do
					local cosmetic = CosmeticLibrary.Cosmetics[k]
					if cosmetic and (cosmetic.Type == "Charm" or k:lower():find("charm")) then proxy[k] = v_ end
				end
			end
			return setmetatable(proxy, {__index = function(t, k)
				local cosmetic = CosmeticLibrary.Cosmetics[k]
	...
	DataController.Get = function(self, key)
		...
		if key == "CosmeticInventory" then
			...
			if data then
				for k, v_ in pairs(data) do
					local cosmetic = CosmeticLibrary.Cosmetics[k]
					if cosmetic and (cosmetic.Type == "Dance" or cosmetic.Type == "Emote" or k:lower():find("dance") or k:lower():find("emote")) then proxy[k] = v_ end
				end
			end
			return setmetatable(proxy, {__index = function(t, k)
				local cosmetic = CosmeticLibrary.Cosmetics[k]
	...
	DataController.Get = function(self, key)
		...
		if key == "CosmeticInventory" then
			...
			if data then
				for k, v_ in pairs(data) do
					local cosmetic = CosmeticLibrary.Cosmetics[k]
					if cosmetic and (cosmetic.Type == "Wrap" or cosmetic.Type == "Wrapping" or k:lower():find("wrap")) then proxy[k] = v_ end
				end
			end
			return setmetatable(proxy, {__index = function(t, k)
				local cosmetic = CosmeticLibrary.Cosmetics[k]
